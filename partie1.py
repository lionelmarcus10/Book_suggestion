    ...
    for options in modifioptions:
        if options == 1:
            ...
            with open('booksread.txt','r',encoding='utf-8') as booksread:
                booksread2 = booksread.readlines()
            with open('booksread.txt','w',encoding='utf-8') as booksread:
                for lecteurs in range(0,len(booksread2)):
                    booksread2[lecteurs] = booksread2[lecteurs].split(',')
                    if booksread2[lecteurs][0] == nommodif :
                        booksread2[lecteurs][0] = npseudomod
                    booksread2[lecteurs] = ','.join(booksread2[lecteurs])
                    booksread.write(booksread2[lecteurs])
